...
console.log("Starting asd-ser-service service...");
...
const start = async () => {
  process.title = "node-asd8-ser-service";
  ...
  try {
    ...
    await setRedisData("asd8-ser-service:start", today.toUTCString());
    console.log(
      "Service start datetime is written to Redis key asd8-ser-service:start",
      today.toUTCString(),
    );
  } catch (err) {
  ...
  console.log(
    "asd-ser-service is listening HTTP/REST port " + servicePort.toString(),
  );
  console.log(
...
/* starting the server asd-ser-service .... */
start();
